...
def _par_tokenize_doc(doc):
    ...
    return item, input_ids, token_type_ids


def encode_all_items(
    model: RecformerModel, tokenizer: RecformerTokenizer, tokenized_items, args
):
    ...
    with torch.no_grad():
        for i in tqdm(
            range(0, len(items), args.batch_size), ncols=100, desc="Encode all items"
        ):
...
def train_one_epoch(
    model, dataloader, optimizer, scheduler, scaler: torch.cuda.amp.GradScaler, args
):
...
def main():
    ...
    parser.add_argument("--first_stage_start_epoch", type=int, default=0)
    parser.add_argument("--second_stage_start_epoch", type=int, default=0)
    ...
    if path_tokenized_items.exists():
        ...
        tokenized_items = {
            item2id[item]: [input_ids, token_type_ids]
            for item, input_ids, token_type_ids in doc_tuples
        }
    ...
    val_data = RecformerEvalDataset(
        train, val, test, mode="val", collator=eval_data_collator
    )
    test_data = RecformerEvalDataset(
        train, val, test, mode="test", collator=eval_data_collator
    )
    ...
    dev_loader = DataLoader(
        val_data, batch_size=args.batch_size, collate_fn=val_data.collate_fn
    )
    ...
    if path_item_embeddings.exists():
        ...
        item_embeddings = encode_all_items(
            model.longformer, tokenizer, tokenized_items, args
        )
    ...
    if args.first_stage_start_epoch > 0:
        model.load_state_dict(torch.load(path_ckpt))

    model.to(args.device)  # send item embeddings to device

    num_train_optimization_steps = (
        int(len(train_loader) / args.gradient_accumulation_steps)
        * args.num_train_epochs
    )
    optimizer, scheduler = create_optimizer_and_scheduler(
        model, num_train_optimization_steps, args
    )
    ...
    for epoch in range(args.first_stage_start_epoch, args.num_train_epochs):
    ...
    for epoch in range(args.second_stage_start_epoch, args.num_train_epochs):

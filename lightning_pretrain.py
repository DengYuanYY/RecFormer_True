...
parser.add_argument(
    "--longformer_ckpt", type=str, default="longformer_ckpt/longformer-base-4096.bin"
)
...
def _par_tokenize_doc(tokenizer_glb: RecformerTokenizer, doc):
...
def main():
    ...
    if path_tokenized_items.exists():
        ...
        func = partial(_par_tokenize_doc, tokenizer)
        # pool_func = pool.imap(func=func, iterable=item_attrs.items())
        # doc_tuples = list(
        #     tqdm(
        #         pool_func,
        #         total=len(item_attrs),
        #         ncols=100,
        #         desc=f"[Tokenize] {path_corpus}",
        #     )
        # )
        doc_tuples = [func(doc) for doc in tqdm(item_attrs.items())]
        tokenized_items = {
            item_id: [input_ids, token_type_ids]
    ...
    tokenized_items = json.load(
        open(path_tokenized_items)
    )  # dir_preprocess / f'attr_small.json'))#
    ...
    peft_model = get_peft_model(pytorch_model, config)
    peft_model.print_trainable_parameters()

    # model = LitWrapper(pytorch_model, learning_rate=args.learning_rate)
    model = LitWrapper(peft_model, learning_rate=args.learning_rate)

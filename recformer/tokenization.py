...
class RecformerTokenizer(LongformerTokenizer):
    ...
    def encode_item(self, item):
        ...
        item = list(item.items())[
            : self.config.max_attr_num
        ]  # truncate attribute number
    ...
    def encode(self, items: List[List[List[str]]], encode_item=True):
        """
        ...
        # items: (length of each clickstream, 2, feature_len)
        items = items[::-1]  # reverse items order
        items = items[
            : self.config.max_item_embeddings - 1
        ]  # truncate the number of items, -1 for <s>
        ...
        for item_idx, item in enumerate(items):
            ...
            item_position_ids += [item_idx + 1] * len(
                item_input_ids
...
if __name__ == "__main__":
    ...
    tokenizer = RecformerTokenizer.from_pretrained(

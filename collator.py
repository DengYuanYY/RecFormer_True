...
class PretrainDataCollatorWithPadding:
    ...
    def __call__(
        ...
        batch_encode_features_a = self.encode_features(batch_feature_a)  # List of Dict
        batch_encode_features_b = self.encode_features(batch_feature_b)  # List of Dict
        batch_a = self.tokenizer.padding(
            batch_encode_features_a, pad_to_max=False
        )  # Dict of Tensor
        batch_b = self.tokenizer.padding(
            batch_encode_features_b, pad_to_max=False
        )  # Dict of Tensor

        batch_a["mlm_input_ids"], batch_a["mlm_labels"] = self.mask_mlm(
            batch_encode_features_a
        )
        batch_b["mlm_input_ids"], batch_b["mlm_labels"] = self.mask_mlm(
            batch_encode_features_b
        )
    ...
    def sample_pairs(
        self, batch_item_ids: List[Dict[str, List[str]]]
    ) -> Tuple[List[List[str]], List[List[str]]]:
        """Split the item ids into two parts and return two sequences
        Shape: (batch_size, length of first split), (batch_size, length of second split)
        """
    ...
    def extract_features(
        self, batch_item_seq: List[List[str]]
    ) -> List[List[List[int]]]:
        """Return features (input_ids, token_type_ids) for each item in the batch_item_seq
        Shape: (batch_size, length of each clickstream, 2, feature_len)
        """
    ...
    def encode_features(
        self, batch_feature: List[List[List[List[str]]]]
    ) -> List[Dict[str, List[int]]]:
        """Encode the features and return a list of encoded features, \
        each feature is a dict of input_ids, item_position_ids, token_type_ids, attention_mask and global_attention_mask
        Shape: (batch_size, Dict)
        """

        # batch_encode_features: (batch_size, length of each clickstream, 2, feature_len)
    ...
    def mask_mlm(self, flat_features: List[Dict[str, List[int]]]):
    ...
    def _collate_batch(
        self, examples: List[List[int]], pad_to_multiple_of: Optional[int] = None
    ):
        ...
        result = examples[0].new_full(
            [len(examples), max_length], self.tokenizer.pad_token_id
        )
